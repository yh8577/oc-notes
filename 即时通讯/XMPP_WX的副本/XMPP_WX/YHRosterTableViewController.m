...
- (IBAction)addFriends:(id)sender {
    [[YHManagerStream sharedInstance].xmppRoster addUser:[XMPPJID jidWithUser:@"zhansan" domain:@"www.huig.com" resource:@"iOS_iphone 11"] withNickname:@"好友便天下"];
}
...
- (void)xmppRoster:(XMPPRoster *)sender didReceivePresenceSubscriptionRequest:(XMPPPresence *)presence {
    // 同意添加对方为好友
    [[YHManagerStream sharedInstance].xmppRoster acceptPresenceSubscriptionRequestFrom:[XMPPJID jidWithUser:@"zhansan" domain:@"www.huig.com" resource:@"iOS_iphone 11"] andAddToRoster:YES];
}
...
- (void)controllerDidChangeContent:(NSFetchedResultsController *)controller {
    self.rosterArrs = self.fetchedResultsController.fetchedObjects;
    [self.tableView reloadData];
    NSLog(@"self.rosterArrs.count =============%zd",self.rosterArrs.count);
}
